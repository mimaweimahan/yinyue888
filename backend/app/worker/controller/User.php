...
use think\facade\Db;
class User extends \app\WorkerInit
{
    protected $model;
    // 初始化
    ...
    public function index()
    {
        ...
        $wallet =  UserWallet::alias('w')
            ...
            ->where('del_time','=',0)
            ->where('type_id','=',1)
            ->where('u.worker_id','=',$this->worker_id)
            ->field([
                // 统计多个字段的和，并重命名
        ...
        $attach = [
            'total'=>UserModel::where('del_time',0)->where('worker_id',$this->worker_id)->where('type_id',1)->count(),
        ...
        if($this->request->isAjax()){
            ...
            $where[] = ['user.worker_id','=',$this->worker_id];
            $_whereRaw = '';
            if($user_type){
        ...
        $is_account = Salesman::where(['worker_id'=>$this->worker_id])->value('is_account');
        if($is_account==1){
            $cols[] = ['title'=>'上分操作','id'=>'balance'];
        }
        $this->assign("cols", json_encode($cols,256));
        $this->assign($param);
    ...
    public function add()
    {
        ...
        if( $this->request->isPost() ){
            ...
            $referee_id = $this->request->param('referee_id',0,'intval');
            $user_type  = $this->request->param('user_type',1,'intval');
    ...
    public function edit()
    {
        ...
        if( $this->request->isPost() ){
            ...
            if(UserModel::update($param,['id'=>$id])){
    ...
    public function delete()
    {
        ...
        if (UserModel::del($_ids,['worker_id'=>$this->worker_id])) {
