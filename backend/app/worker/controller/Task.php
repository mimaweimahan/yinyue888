...
class Task extends \app\WorkerInit
{
    protected $model;
    // 初始化
    ...
    public function add() {
        if($this->request->isPost()){
            ...
            //任务编号是否重复
            $be = TaskTpl::getToArray(['worker_id'=>$this->worker_id,'task_no'=>$params['task_no']]);
            if($be){
    ...
    public function edit() {
        ...
        if($this->request->isPost()){
            ...
            //任务编号是否重复
            $be = TaskTpl::getToArray(['worker_id'=>$params['worker_id'], 'task_no'=>$params['task_no']]);
            if($be){
                return self::error('任务编号已存在');
            }
