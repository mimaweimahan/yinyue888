        ...
        table.on('toolbar(table_list)', function(obj){
            ...
            if(_event === 'delete'){
                ...
                layer.confirm('确定要删除选中的 ' + data.length + ' 条链接吗？', function(index){
                    ...
                    $.post("{:url('app_download/del')}", {ids: ids}, function(res){
                        layer.msg(res.msg);
                        if(res.code === 1){
                            layer.close(index);
                            table.reload('table_list',{});
        ...
        table.on('tool(table_list)', function(obj){
            ...
            if(_event === 'edit'){
                layer.open({
                    ...
                    maxmin: true,
                    area: ['600px', '300px'],
                    content: "{:url('app_download/edit')}?id=" + data.id,
                    end:function(){
                        table.reload('table_list',{});
                    }
                });
            ...
            if(_event === 'del'){
                layer.confirm('确定要删除这条链接吗？', function(index){
                    $.post("{:url('app_download/del')}", {id: data.id}, function(res){
                        layer.msg(res.msg);
                        if(res.code === 1){
                            layer.close(index);
                            table.reload('table_list',{});
