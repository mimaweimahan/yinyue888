...
<head>
    ...
    <title>{if !empty($id)}编辑事件{else}添加事件{/if}</title>
    ...
    <form class="layui-form" method="post" action="{if !empty($id)}{:url('edit')}{else}{:url('add')}{/if}">
        ...
        <div class="form-item">
            ...
            <div class="input-block">
                <textarea name="content" id="content" placeholder="请输入事件内容" class="layui-textarea" lay-verify="required" style="min-height:400px">{$content|default=''}</textarea>
            </div>
        </div>
