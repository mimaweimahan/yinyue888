...
<head>
    ...
    <title>{$config['config_app_name']}</title>
...
<div class="layui-layout layui-layout-admin">
    <div class="layui-header">
        <div class="layui-logo">
            <a href="javascript:void(0);" class="logo">
                <!-- 迷你模式下Logo的大小为50X50 -->
                <span class="logo-mini"><img src="{:getConfig('config_logo_sys')}" width="40" /></span>
                <!-- 普通模式下Logo -->
                <span class="logo-lg"><img src="{:getConfig('config_logo_sys')}" height="100%" /></span>
            </a>
        </div>
        ...
        <ul class="layui-nav layui-layout-right ">
            <li class="layui-nav-item">
                <a href="javascript:void(0);" >
                    <img src="{:url('tool/avatar/index',['uid'=>$admin_info['uid']])}" class="layui-nav-img layui-hide-xs">
                    <i class="layui-hide-xs">{$admin_info['nickname']}</i>
                </a>
                <dl class="layui-nav-child">
                    <dd><a href="{:url('admin/admin/update')}" target="main">基本资料</a></dd>
                    <dd><a href="{:url('admin/index/cache')}" target="main">清除缓存</a></dd>
                    <dd><a href="{:url('admin/admin/binding')}?id={$admin_id}" target="main">绑定谷歌</a></dd>
                    <dd><a href="{:url('admin/login/signout')}">安全退出</a></dd>
                </dl>
            </li>
        </ul>
    </div>
    <div class="left-user-box">
        <a href="{:url('admin/admin/update')}" target="main">
            <img src="{:url('tool/avatar/index',['uid'=>$admin_info['uid']])}" class="layui-nav-img">
            <dl>
                <dd>{$admin_info['nickname']}</dd>
                <dt>{$admin_info['phone']}</dt>
            </dl>
        </a>
    </div>
    ...
    <div class="layui-body">
        <div class="main-panel" style="width:100%; height: 100%;">
            <iframe src="{:url('main')}" name="main" id="frame-main" width="100%" height="100%" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling-x="no" scrolling-y="auto" allowtransparency="yes"></iframe>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    layui.config({
        ...
        var left_menu_json = {$left_menu_json|raw};
        if(left_menu_json){
