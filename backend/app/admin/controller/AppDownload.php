...
class AppDownload extends \app\AdminInit
{
    protected $model;
    ...
    protected function initialize(){
        parent::initialize();
        $this->model = new AppDownloadModel();
    }
    ...
    public function index() {
        ...
        if($this->request->isAjax()){
            $data = AppDownloadModel::order(['id'=>'desc'])
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if(!$url){
                return self::error('请填写下载链接！');
            }
            // 验证URL格式
            if(!filter_var($url, FILTER_VALIDATE_URL)){
                return self::error('下载链接格式不正确！');
            }
            ...
            $result = AppDownloadModel::create($data);
            if($result){
    ...
    public function edit()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $result = AppDownloadModel::where('id', $id)->update($data);
            if($result){
        ...
        $data = AppDownloadModel::getToArray(['id'=>$id]);
        if(!$data){
            return self::error('链接不存在！');
        }
        $data['id'] = $id; // 确保传递 id
        $this->assign($data);
    ...
    public function del()
    {
        ...
        if(!empty($ids) && is_array($ids)){
            $result = AppDownloadModel::destroy($ids);
            if($result){
        ...
        $result = AppDownloadModel::destroy($id);
        if($result){
