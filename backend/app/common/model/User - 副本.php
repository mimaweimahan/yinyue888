...
class User extends Model
{
    ...
    public static function register(array $opt=[], int $user_type = 0, int $referee_id=0){
        ...
        if(isset($opt['email']) && $opt['email']) {
            $lockKeys[] = 'register_email_' . md5($opt['email']);
        }
        if(isset($opt['phone']) && $opt['phone']) {
            $lockKeys[] = 'register_phone_' . md5($opt['country_code'] . '_' . $opt['phone']);
        }

        // 尝试获取分布式锁（防止并发注册）
        $cache = \think\facade\Cache::store('redis');
        $lockAcquired = [];
        $lockTimeout = 10; // 锁超时时间10秒
        ...
        try {
            ...
            foreach ($lockKeys as $lockKey) {
                ...
                if ($cache->has($lockKey)) {
                    // 锁已存在，说明有其他请求正在处理
                    throw new \Exception('register_locked');
                }
                $cache->set($lockKey, $lockValue, $lockTimeout);
                $lockAcquired[$lockKey] = $lockValue;
            }

            // 开启事务
            Db::startTrans();
            try {
                // 判断用户是否存在（使用 FOR UPDATE 锁定）
                if(isset($opt['email']) && $opt['email']) {
                    $be = self::where('email', $opt['email'])->lock(true)->value('id');
                    if($be){
                        Db::rollback();
                        return -8; # 邮箱已存在
                    }
                ...
                if(isset($opt['phone']) && $opt['phone']) {
                    $be = self::where('phone', $opt['phone'])
                        ->where('country_code', $opt['country_code'])
                        ->lock(true)
                        ->value('id');
                    if($be){
                        Db::rollback();
            ...
            if ($e->getMessage() === 'register_locked') {
                return -14; # 注册请求过于频繁，请稍后再试
            }
            throw $e;
        } finally {
            // 释放所有锁
            foreach ($lockAcquired as $lockKey => $lockValue) {
                // 只释放自己持有的锁
