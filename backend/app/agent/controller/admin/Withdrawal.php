<?php

namespace app\agent\controller\admin;
use app\agent\model\Salesman;
use app\agent\model\Withdrawal as WithdrawalModel;
use app\common\model\user\UserWallet;
use app\common\traits\ControllerTrait;

class Withdrawal extends \app\AdminInit
{
    protected $model;
    // 初始化
    protected function initialize(){
        parent::initialize();
        $this->model = new WithdrawalModel();
    }
    use ControllerTrait;

    /**
     * 列表
     */
    public function index() {
        $where = [];
        $params    = $this->request->param();
        $keys      = $this->request->param('keys','','trim');
        $agent_id  = $this->request->param('agent_id',0,'intval');
        $worker_id = $this->request->param('worker_id',0,'intval');
        $uid       = $this->request->param('uid',0,'intval');
        $user_type = $this->request->param('user_type',0,'intval');
        //处理状态 0待处理，1已成功，2已取消
        $status    = $this->request->param('status',0,'intval');

        // 账号类型
        // 0TRC，1ERC ，2BSC
        $withdraw_type = $this->request->param('withdraw_type',0,'intval');
        $limit     = $this->request->param('limit',10,'intval');
        if($this->request->isAjax()){

            // 开始时间
            $start_time = $this->request->param('start_time','','trim');
            // 结束时间
            $end_time   = $this->request->param('end_time','','trim');
            $_date = [];
            if ($start_time) {
                $_date = ['created_at','>=', strtotime($start_time)];
            }
            if ($end_time) {
                $_date = ['created_at','<=', strtotime($end_time)];
            }
            if ($end_time && $start_time ) {
                $_date = [ ['created_at','>=', strtotime($start_time)], ['created_at','<=', strtotime($end_time)] ];
            }
            if($_date){
                $where[] = $_date;
            }
            //++++++++++++++++++

            if($agent_id){
                $where[] = ["agent_id","=",$agent_id];
            }
            if($worker_id){
                $where[] = ["worker_id","=",$worker_id];
            }
            if($uid){
                $where[] = ["uid","=",$uid];
            }
            if($user_type){
                $where[] = ["user_type","=",$user_type-1];
            }
            if($withdraw_type){
                $where[] = ["withdraw_type","=",$withdraw_type-1];
            }
            if($status){
                $where[] = ["status","=",$status-1];
            }
            $data = WithdrawalModel::where($where)
                ->order(['created_at'=>'DESC'])
                ->paginate(['list_rows' => $limit,'query' =>$params], false)->toArray();

            $attach = [
                'all_total'=>count($data['data']),
                'all_balance'=>WithdrawalModel::where($where)->sum('balance')
            ];
            return self::result_layui($data,$attach);
        }
        $this->assign("keys", $keys);
        $this->assign("url", url('index'));
        $this->assign("limit", $limit);
        $this->assign("salesman_list", Salesman::getList());
        return $this->fetch();
    }

    /**
     * 提现审核
     * @return string
     */
    public function approval(){
        $id = $this->request->param('id', 0, 'intval');
        if ($id == 0) {
            return self::error('缺少ID！');
        }
        if ($this->request->isPost()) {
            $status = $this->request->param('status', 0, 'intval');
            $remark = $this->request->param('mark', '', 'trim');
            if(!$status){
                return self::error('请选择审批结果！');
            }
            if(!$remark){
                //return self::error('请填写审批说明！');
            }
            $_data = WithdrawalModel::getToArray(['id'=>$id]);
            if(!$_data){
                return self::error('充值信息不存在！');
            }

            if($_data['status']>0){
                return self::error('已经处理过了不能重复处理！');
            }
            $f_data = ['actype'=> 0];

            if($status == 2){
                $rt = UserWallet::updateAc($_data['uid'],['balance'=>$_data['balance']],6,'提现退回', $f_data);
                if($rt['code']==0){
                    return self::error($rt['msg']);
                }
                $do = WithdrawalModel::update([
                    'status'=>$status,
                    'mark'=>$remark
                ],['id'=>$id]);
            }else{
                $do = WithdrawalModel::update([
                    'status'=>$status,
                    'is_paid'=>1,
                    'mark'=>$remark,
                    'payTime'=>time(),
                ],['id'=>$id]);
            }
            if ($do) {
                return self::success('操作成功！');
            }
            return self::error('操作失败！');
        }
        $result = $this->model::getToArray(['id'=>$id]);
        if(!isset($result)){
            return self::error('信息不存在！');
        }
        $this->assign($result);
        return $this->fetch();
    }
}