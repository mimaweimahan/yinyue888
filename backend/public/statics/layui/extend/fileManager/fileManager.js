layui.define(['jquery', 'layer', 'laypage'], function (exports) { //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
    ...
    Class.prototype.render = function () {
        ...
        if (options.btn_create) {
            _btn += '<button type="button" class="layui-btn layui-bg-orange layui-btn-sm" id="new_dir">新增分类</button>';
        }
        if (options.btn_upload) {
            _btn += '<button type="button" class="layui-btn layui-btn-sm" id="uploadfile">上传文件</button>';
        }
        if (options.btn_move) {
            _btn += '<button type="button" class="layui-btn layui-btn-primary layui-btn-sm" id="move_file">移动</button>';
        }
        if (options.btn_del) {
            _btn += '<button type="button" class="layui-btn layui-btn-primary layui-btn-sm" id="del_file">删除</button>';
        }
    ...
    Class.prototype.events = function () {
        ...
        that.layToolBar.on('click', '#new_dir', function () {
            ...
            layer.prompt({ title: '请输入分类名字', formType: 0 }, function (name, index) {
                layer.close(index);
                //新建文件夹
                layui.event.call(this,
                    'fileManager', eventType + '(' + filter + ')'
        ...
        //删除
        that.layToolBar.on('click', '#del_file', function () {
            var othis = $(this);
            let eventType = 'del';
            layui.event.call(this,
                'fileManager', eventType + '(' + filter + ')'
        ...
        //移动文件夹
        that.layToolBar.on('click', '#move_file', function () {
            var othis = $(this);
            let eventType = 'move';
            layui.event.call(this,
                'fileManager', eventType + '(' + filter + ')'
