<?php /*a:1:{s:71:"/www/wwwroot/tisktshop.com/app/agent/view/admin/withdrawal/approval.php";i:1762764658;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
<div class="page-package">
    <form class="layui-form" method="post" action="<?php echo url('approval'); ?>">
            ...
            <div class="input-inline">
                <div style="width: 180px">
                    <select name="status" id="status" class="layui-select">
                        ...
                        <option value="1" <?php if($status == 1): ?> selected <?php endif; ?>>审批通过并已给用户付款</option>
                        <option value="2" <?php if($status == 2): ?> selected <?php endif; ?>>拒绝提现申请</option>
                    </select>
                </div>
            </div>
        </div>
        <div class="form-item">
            ...
            <div class="input-block">
                <textarea name="mark" id="mark" placeholder=" 填写审批说明 " class="layui-textarea"><?php echo html_entities($mark); ?></textarea>
            </div>
        </div>
        <div class="footer-btn-box" <?php if($status==1): ?> style="display: none;" <?php endif; ?>>
        <div>
        ...
        form.on('submit(submit_btn)', function (data) {
            $.ajax({
                ...
                success: function(data){
                    layer.msg(data.msg);
                    if(data.code === 1){
                        setTimeout(function () {  layer.closeAll(); location.reload(); },3000);
                    }
                }
            });
            return false;
