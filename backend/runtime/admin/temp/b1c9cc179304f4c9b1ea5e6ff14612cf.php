<?php /*a:1:{s:58:"/www/wwwroot/tisktshop.com/app/admin/view/notice/index.php";i:1766689772;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    ...
    <div class="panel-body">
        <div class="layui-show">
            <form class="layui-form">
                <ul class="search-box">
                    <li><input type="text" name="keys" value="<?php echo html_entities($keys); ?>" placeholder="请输入公告标题关键词" autocomplete="off" class="layui-input"></li>
        ...
        table.on('toolbar(table_list)', function(obj){
            ...
            if(_event === 'delete'){
                ...
                layer.confirm('确定要删除选中的 ' + data.length + ' 条公告吗？', function(index){
                    ...
                    $.post("<?php echo url('notice/del'); ?>", {ids: ids}, function(res){
                        layer.msg(res.msg);
                        if(res.code === 1){
                            layer.close(index);
                            table.reload('table_list',{});
        ...
        table.on('tool(table_list)', function(obj){
            ...
            if(_event === 'edit'){
                layer.open({
                    ...
                    maxmin: true,
                    area: ['800px', '600px'],
                    content: "<?php echo url('notice/edit'); ?>?id=" + data.id,
                    end:function(){
                        table.reload('table_list',{});
                    }
                });
            ...
            if(_event === 'del'){
                layer.confirm('确定要删除这条公告吗？', function(index){
                    $.post("<?php echo url('notice/del'); ?>", {id: data.id}, function(res){
                        layer.msg(res.msg);
                        if(res.code === 1){
                            layer.close(index);
                            table.reload('table_list',{});
