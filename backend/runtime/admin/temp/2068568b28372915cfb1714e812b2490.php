<?php /*a:1:{s:59:"/www/wwwroot/tisktshop.com/app/admin/view/config/config.php";i:1661007164;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
<div class="panel-default">
    <div class="panel-heading">
        ...
        <ul class="panel-tab">
            <?php if($id == 0): ?>
            <li class="layui-this"><a href="<?php echo url('config'); ?>"> 全部</a></li>
            <?php else: ?>
            <li><a href="<?php echo url('config'); ?>"> 全部</a></li>
            <?php endif; if(is_array($config_group_list) || $config_group_list instanceof \think\Collection || $config_group_list instanceof \think\Paginator): $i = 0; $__LIST__ = $config_group_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;if($id == $key): ?>
            <li class="layui-this"><a href="<?php echo url('config',['id'=>$key]); ?>"> <?php echo html_entities($group); ?>配置</a></li>
            <?php else: ?>
            <li><a href="<?php echo url('config',['id'=>$key]); ?>"> <?php echo html_entities($group); ?>配置</a></li>
            <?php endif; ?>
...
<script type="text/html" id="set_panel">
    <a href="<?php echo url('edit'); ?>?id={{ d.id }}" type="main" class="layui-btn layui-btn-xs layui-btn-primary">编辑</a>
    <a class="layui-btn layui-btn-xs layui-btn-primary" lay-event="del">删除</a>
</script>
<script type="text/html" id="toolbar">
    <div class="layui-btn-container">
        <a href="<?php echo url('add',['id'=>$id]); ?>" target="main" class="layui-btn layui-btn-sm"><span><i class="layui-icon layui-icon-add-circle"></i>新增</span></a>
...
<script type="text/javascript">
    layui.config({
        ...
        table.render({
            elem:'#table_list'
            ,height: table_h
            ,limit: table_h>800?30:"<?php echo html_entities($limit); ?>"
        ...
        $("#add_rule").on('click',function () {
            layer.open({
                ...
                content:"<?php echo url('add',['id'=>$id]); ?>",
                end:function () {
                    //location.reload();
                    table.reload('table_list',{});
                }
