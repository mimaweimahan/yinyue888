<?php /*a:1:{s:56:"/www/wwwroot/tisktshop.com/app/admin/view/notice/add.php";i:1766683201;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    ...
    <title><?php if(!empty($id)): ?>编辑公告<?php else: ?>添加公告<?php endif; ?></title>
    ...
    <form class="layui-form" method="post" action="<?php if(!empty($id)): ?><?php echo url('edit'); else: ?><?php echo url('add'); ?><?php endif; ?>">
        ...
        <div class="form-item">
            ...
            <div class="input-block">
                <input type="text" name="title" value="<?php echo html_entities((isset($title) && ($title !== '')?$title:'')); ?>" id="title" placeholder="请输入公告标题" autocomplete="off" class="layui-input" lay-verify="required" />
            </div>
        </div>
        <div class="form-item">
            <label for="content" class="form-label">公告内容 <span style="color:red">*</span></label>
            <div class="input-block">
                <textarea name="content" id="content" placeholder="请输入公告内容" class="layui-textarea" lay-verify="required" style="min-height:150px"><?php echo html_entities((isset($content) && ($content !== '')?$content:'')); ?></textarea>
            </div>
        </div>
        <div class="form-item">
            ...
            <div class="input-inline">
                <div style="width: 180px">
                    <select name="status" id="status" class="layui-select">
                        <option value="0" <?php if(isset($status) && $status == 0): ?> selected <?php endif; ?>>隐藏</option>
                        <option value="1" <?php if(isset($status) && $status == 1): ?> selected <?php endif; ?>>显示</option>
                    </select>
                </div>
            </div>
        </div>
            ...
            <div class="input-inline">
                <div style="width: 180px">
                    <select name="show_home" id="show_home" class="layui-select">
                        <option value="0" <?php if(isset($show_home) && $show_home == 0): ?> selected <?php endif; ?>>否</option>
                        <option value="1" <?php if(isset($show_home) && $show_home == 1): ?> selected <?php endif; ?>>是</option>
                    </select>
                </div>
            </div>
        </div>
        <div class="form-item">
            ...
            <div class="input-inline">
                <div style="width: 180px">
                    <input type="number" name="sort" value="<?php echo html_entities((isset($sort) && ($sort !== '')?$sort:0)); ?>" id="sort" placeholder="数字越小越靠前" autocomplete="off" class="layui-input" />
                </div>
            </div>
        </div>
