<?php /*a:1:{s:59:"/www/wwwroot/tisktshop.com/app/admin/view/user/index/xj.php";i:1762348844;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
<script type="text/javascript">
    ...
    // 相关常量
    const ACCOUNT_API = "<?php echo url('agent/admin.wallet/index'); ?>";
    layui.config({
        version:1.0,
        ...
        table.render({
            ...
            ,cols: [[
                {
                    title: "ID", align: "center", field: "id",width: 90,
                    templet: function (d) {
                        var str = d.id + '<br /><br />';
                ...
                {field: 'updated_time', align: 'left', sort:true, title: '活跃信息', width: 200, templet: function(d){
                        var str = '<span class="layui-badge-rim layui-bg-green">登录IP </span> ' + (d.last_login_ip ? d.last_login_ip : d.reg_ip) +'<hr class="mr5" />';
                        str += '<span class="layui-badge-rim layui-bg-orange">地区 </span> ' +( d.last_login_ip ? d.last_area : d.reg_ip) +'<hr class="mr5" />';
                        str += (d.updated_time  ? '<span class="layui-badge-rim layui-bg-purple">活跃 </span> ' + d.is_online_time :  '<span class="layui-badge-rim">注册</span>' + d.reg_time) ;
                        return  str;
                    }
                },
                {
                ...
                {
                    title: "手机/邮箱/注册时间", align: "center", field: "created_at", width: 200,sort:true,
                    templet: function(d){
                        switch (d.user_type) {
                            case 1 :
                                return  '<b class="color-blue" title="'+d.nickname+'">+' + (d.country_code+' '+ d.phone ? d.country_code+' '+ d.phone : '-') + '</b><hr class="mr5" />' + (d.email ? d.email : '-') + '<hr class="mr5" />' + d.reg_time;
                            default :
                                return '+' + '<span title="'+d.nickname+'">'+(d.country_code+' '+ d.phone ? d.country_code+' '+ d.phone : '-')+'</span>' +'<hr class="mr5" />' + (d.email ? d.email : '-') + '<hr class="mr5" />' + d.reg_time;

                        }
                    }
                },
                {field: 'invite',align: 'center',title: '邀请码', sort:true,width: 110},
                {field: 'balance',align: 'center',title: '账户余额', sort:true,width: 110},
                {field: 'freeze_balance',align: 'center',title: '交易/差额',width: 100,templet: function(d){
                ...
                {field: 'in_balance',align: 'center',title: '充值',sort:true, width: 100},
                {field: 'out_balance',align: 'center',title: '提现',sort:true, width: 100},
                ...
                {field: 'invite',align: 'left',title: '分享收益', sort:true,width: 120,templet: function(d){
                ...
                {field: 'is_ip_repeat',align: 'center',event:"see",width:150,title: '备注',templet: function(d){
                ...
                {field: 'is_top',align: 'center', title: '置顶', width: 100,toolbar: '#is_top'},
                {title: "操作", toolbar: "#table-bar", align: "center", width: 120,}
            ]]
                    ...
                    ,click: function(res){
                        ...
                        if (res.id === 'balance'){
                            ...
                            layer.open({
                                ...
                                maxmin: true,
                                area: [is_mobile() ? "100%" : "750px", is_mobile() ? "100%" : "600px"],
                                content: "<?php echo url('agent/admin.pai/index'); ?>?id=" + value,
                            });
                            ...
                            layer.open({
                                ...
                                maxmin: true,
                                area: [is_mobile() ? "100%" : "750px", is_mobile() ? "100%" : "600px"],
                                content: "<?php echo url('gg'); ?>?id=" + value,
                            });
                        }  else if (res.id === 'edit')  {
                            let value = data.id;
                            layer.open({
