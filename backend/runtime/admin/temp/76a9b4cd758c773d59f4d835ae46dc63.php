<?php /*a:1:{s:64:"/www/wwwroot/tisktshop.com/app/admin/view/app_download/index.php";i:1766949832;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
<div class="panel-default">
    <div class="panel-heading">
        <div class="panel-lead"><em>APP下载链接管理</em>管理APP下载链接，供前端调用</div>
    </div>
...
<script type="text/html" id="url_panel">
...
<script type="text/html" id="toolbar">
    <div class="layui-btn-container">
        <div class="layui-btn-group">
            <button type="button" class="layui-btn layui-btn-sm" lay-event="add"><span><i class="layui-icon layui-icon-add-circle"></i>添加链接</span></button>
    ...
    layui.config({
        ...
        table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'url', title:'下载链接', minWidth:500, toolbar: '#url_panel'}
        ...
        table.on('toolbar(table_list)', function(obj){
            ...
            if(_event === 'add'){
                layer.open({
                    ...
                    maxmin: true,
                    area: ['600px', '300px'],
                    content: "<?php echo url('app_download/add'); ?>",
                    end:function(){
            ...
            if(_event === 'delete'){
                ...
                layer.confirm('确定要删除选中的 ' + data.length + ' 条链接吗？', function(index){
                    var ids = [];
                    for(var i = 0; i < data.length; i++){
                        ids.push(data[i].id);
                    }
                    $.post("<?php echo url('app_download/del'); ?>", {ids: ids}, function(res){
                        layer.msg(res.msg);
                        if(res.code === 1){
                            layer.close(index);
                            table.reload('table_list',{});
        ...
        table.on('tool(table_list)', function(obj){
            ...
            if(_event === 'edit'){
                layer.open({
                    ...
                    maxmin: true,
                    area: ['600px', '300px'],
                    content: "<?php echo url('app_download/edit'); ?>?id=" + data.id,
                    end:function(){
                        table.reload('table_list',{});
                    }
                });
            ...
            if(_event === 'del'){
                layer.confirm('确定要删除这条链接吗？', function(index){
                    $.post("<?php echo url('app_download/del'); ?>", {id: data.id}, function(res){
                        layer.msg(res.msg);
                        if(res.code === 1){
                            layer.close(index);
                            table.reload('table_list',{});
